...
class Img2Shape(LightningModule):
    def __init__(self,
                 img_encoder: torch.nn.Module,
                 intermediate_dim, shape_num, ln_eps, drop_rate,
                 learning_rate,
                 warmup_tokens,
                 final_tokens,
                 weight_decay,
        ...
        self.ln = nn.LayerNorm(img_encoder.hidden_dim, eps=ln_eps)
        self.img_proj = nn.Linear(img_encoder.hidden_dim, intermediate_dim, bias=False)
        self.dropout = nn.Dropout(drop_rate)
        self.classifier = nn.Sequential(
            nn.Linear(intermediate_dim, shape_num)
        )
    
    def forward(self, img):
        img_hidden = self.img_encoder.forward(img)

        img_representation = self.ln(img_hidden)
        ...
        y = self.dropout(img_representation)
        y = self.classifier(y)
    ...
    def training_step(self, batch, batch_idx):
        ...
        accuracy = multilabel_accuracy(
            logits, target,
            num_labels=self.hparams.shape_num,
            threshold=self.hparams.threshold,
            average='macro'
        )
        self.tokens += 17 * target.shape[0]
    ...
    def load_from_pretrain(self, encoder_state_dict, proj_state_dict, ln_state_dict):
        ...
        if proj_state_dict is not None:
            self.img_proj.load_state_dict(proj_state_dict)
