...
class ImgTextDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        target = text['input_ids'][0][1:]
        text = {
            'input_ids': text['input_ids'][0][:-1],
            'attention_mask': text['attention_mask'][0][:-1]
        }

        return img, text, target
    
class ImgTextDataModule(LightningDataModule):
    def __init__(self,
    ...
    def setup(self, stage='fit', train_frac=0.8):
        ...
        self.train_dataset = ImgTextDataset(id=train_id, text=train_text, img_dir=train_img_dir, vocab_path=self.vocab_path, max_text_length=self.max_text_length)
        self.validation_dataset = ImgTextDataset(id=valid_id, text=valid_text, img_dir=valid_img_dir, vocab_path=self.vocab_path, max_text_length=self.max_text_length)
